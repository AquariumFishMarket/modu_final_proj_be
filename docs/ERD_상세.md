...
![ERD](../images/fish_market_diagram.png)
## 1. User - 사용자 계정 정보
| 필드                      | 의미              | 사용 목적                     |
| ----------------------- | --------------- | ------------------------- |
...
| like_count              | 게시글 좋아요 수     | 게시글 인기 표시             |
...
| 필드         | 의미            | 사용 목적         |
| ---------- | ------------- | ------------- |
| id         | PK, 좋아요 고유 ID | 좋아요 기록 식별     |
| user_id    | FK → User.id  | 좋아요 누른 사용자 확인 |
| post_id    | FK → Post.id  | 좋아요 대상 게시글 확인 |
| created_at | 좋아요 생성 시각     | 기록 및 정렬 기준    |

- **사용 시나리오**:
    - 게시글 좋아요/취소, 사용자별 좋아요 조회
...
| title                   | 상품 이름        | 화면 표시               |
| description             | 상품 설명        | 화면 표시               |
| price                   | 가격           | 화면 표시, 구매/결제 처리     |
| report_count            | 신고 누적 카운트    | 블러 처리 판단 기준         |
...
| target_type         | 신고 대상 타입          | 'post', 'comment', 'product', 'message' 등 식별 |
